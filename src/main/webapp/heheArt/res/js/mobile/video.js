function getVideos(){
    callAjax('/heheArtService/getVideos', '', 'getVideosCallback', '', '', '', '');
}
function getVideosCallback(data){
    if (data.status == "ok" && data.callBackData.length > 0) {
        ...
        for (var i = 0; i < data.callBackData.length; i++) {
            ...
            template += '<a data-video-path="' + video.videoPath + '" data-video-type="' + video.type + '" href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg hehe-open-video">';
