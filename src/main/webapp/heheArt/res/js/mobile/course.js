...
function getCoursesCallback(data){
    if (data.status == "ok" && data.callBackData.length > 0) {
        ...
        for (var i = 0; i < data.callBackData.length; i++) {
            ...
            template += '<a data-id="' + course.id + '" href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg hehe-open-popup">';
...
function getOneWeekScheduledCourses(){
    var user = checkUser();
    if(user){
        callAjax('/heheArtService/getOneWeekScheduledCourses', '', 'getOneWeekScheduledCoursesCallback', '', '', 'memberId='+user.id, '');
    }
}
...
function orderCourse(scheduleId){
    ...
    callAjax('/heheArtService/insertOrder', '', 'insertOrderCallback', scheduleId, '', param, '');
}
...
function cancelCourse(orderId, scheduleId){
    callAjax('/heheArtService/deleteOrder', '', 'deleteOrderCallback', scheduleId, '', 'orderId='+orderId, '');
}
