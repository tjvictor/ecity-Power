function getAllNewsBrief(){
    callAjax('/heheArtService/getAllNewsBrief', '', 'getAllNewsBriefCallback', '', '', '', '');
}
function getAllNewsBriefCallback(data){
    if (data.status == "ok" && data.callBackData.length > 0) {
        ...
        for (var i = 0; i < data.callBackData.length; i++) {
            ...
            template += '<a data-news-id="'+news.id+'" href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg hehe-open-popup">';
