function getPhotoWallTotalCount(){
    callAjax('/heheArtService/getPhotoWallTotalCount', '', 'getPhotoWallTotalCountCallback', '', '', '', '', false);
}
...
function getAllPhotoWallWithPhotos(pageNumber, pageSize){
    ...
    callAjax('/heheArtService/getAllPhotoWallWithPhotos', '', 'getAllPhotoWallWithPhotosCallback', '', '', param, '');
}
    ...
    if (data.status == "ok" && data.callBackData.length > 0) {
        ...
        for (var i = 0; i < data.callBackData.length; i++) {
            ...
            for(var j = 0; j < photoWall.photoList.length; j++){
                if(j < 9){
                    template += '           <a class="weui-grid js_grid" style="padding:10px;"><img class="hehe-photo-browse" src="' + photoWall.photoList[j].thumbUrl + '" data-target="' + j + '" alt="" style="width:100%;height:100%" onclick="hehePhotoBrowse(this)"></a>';
                }
